...
class ToolSearchConfig:
    ...
    # Protocol-critical tools whose complete schema remains directly visible.
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        always_visible = tuple(dict.fromkeys(
            str(value).strip() for value in always_visible_values if str(value).strip()
        ))
        return cls(
...
def assemble_tool_defs(tool_defs: List[Dict[str, Any]], *, context_length: Optional[int] = None,
    ...
    if config.always_visible and deferrable:
        ...
        pinned = [td for td in deferrable if _fn(td).get("name") in pinned_names]
        if pinned:
            visible.extend(pinned)
            deferrable = [td for td in deferrable if td not in pinned]
    if not deferrable:
