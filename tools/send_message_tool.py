...
async def _send_via_adapter(platform, pconfig, chat_id, chunk, *, thread_id=None, media_files=None,
    ...
    return await _call_plugin_standalone_sender(
        platform_name, sender, pconfig, chat_id, chunk, thread_id=thread_id,
        media_files=media_files, force_document=force_document,
    )


async def _call_plugin_standalone_sender(
    platform_name, sender, pconfig, chat_id, message, *, thread_id=None,
    media_files=None, force_document=False,
):
    """Invoke a registered sender and enforce its public result contract."""
    try:
        result = await sender(pconfig, chat_id, message, thread_id=thread_id, media_files=media_files,
                              force_document=force_document)
    except asyncio.CancelledError:
        raise
    except Exception as e:
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, args=None):
    ...
    try:
        ...
        pass
    if media_files and entry is not None and getattr(entry, "supports_media_delivery", False):
        if entry.standalone_sender_fn is None:
            return {"error": (f"Plugin platform '{platform_name}' declares media delivery "
                              "but has no standalone_sender_fn")}
        return await _call_plugin_standalone_sender(
            platform_name, entry.standalone_sender_fn, pconfig, chat_id, message,
    ...
    if media_files and platform_name != "buzz":
        if not message.strip():
            return {"error": (f"send_message MEDIA delivery is currently only supported for {_MEDIA_PLATFORMS_NOTE}; "
                              f"plugin platforms may opt in through supports_media_delivery; "
        ...
        warning = (f"MEDIA attachments were omitted for {platform_name}; "
                   ...
                   "or plugin platforms declaring supports_media_delivery")
