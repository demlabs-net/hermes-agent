...
def skill_manage(
    ...
    if (gate_result := _apply_skill_write_gate(action, name, **args)) is not None:
        return gate_result
    # Operator-mounted skills stay immutable in external-state mode. Only the
    # externally backed writable tree may be mutated.
    try:
    ...
    if result.get("success"):
        try:
            ...
            result = {"success": False, "error": f"External skill persistence failed: {exc}"}
    if result.get("success"):
        _record_success(
