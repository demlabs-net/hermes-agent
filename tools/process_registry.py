...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def snapshot_running_ids(self, task_id: str, *, session_key: str = "") -> frozenset[str]:
        """Running IDs owned by a gateway session or ``task_id`` — a turn-boundary marker: on timeout
        only processes absent from the starting snapshot belong to the abandoned
        turn; older ones intentionally span turns and must survive. A session key
        covers local environments whose process task id is collapsed to ``default``."""
        def is_owned(session: ProcessSession) -> bool:
            return session.session_key == session_key if session_key else session.task_id == task_id

        with self._lock:
            return frozenset(s.id for s in self._running.values() if is_owned(s) and not s.exited)

    def kill_started_since(self, task_id: str, baseline_ids, *, source: str, session_key: str = "") -> int:
        """Kill session/task processes created after ``baseline_ids``. Output is
        ...
        return self.kill_all(
            task_id, exclude_ids=frozenset(baseline_ids or ()), source=source,
            consume_output=True, session_key=session_key,
        )

    def kill_all(
        self, task_id: Optional[str] = None, *, exclude_ids: frozenset = frozenset(),
        source: str = "kill_all", consume_output: bool = False, session_key: str = "") -> int:
        ...
        with self._lock:
            targets = [
                s for s in self._running.values()
                if is_owned(s) and s.id not in exclude_ids and not s.exited
            ]
        return sum(
