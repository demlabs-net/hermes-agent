...
def _model_protected_cron_names() -> set[str]:
    """Names the model-facing cron tool may not mutate."""
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            if protected_error := _protected_cron_error(name, "created"):
        ...
        if normalized in {"remove", "pause", "update"}:
            if protected_error := _protected_cron_error(job.get("name"), normalized + "d"):
                return tool_error(protected_error, success=False)
