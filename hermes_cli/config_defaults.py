...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Maximum assistant-text length inspected by the intent-ack detector; 0 removes the ceiling.
        ...
        # Optional regex matched against a successful tool result.
    ...
    # Optional authoritative storage for built-in MEMORY.md/USER.md and the writable skills tree.
    # Existing code uses an ephemeral compatibility cache; only this backend is durable.
    "external_state": {
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        "api_server": {  # OpenAI-compatible API server platform (gateway/platforms/api_server.py).
            ...
            # Stateless task runners may opt in to treating background processes as run-owned
            # scratch work and reap them when /v1/runs reaches a terminal state.
