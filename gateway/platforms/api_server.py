...
def _start_api_run_budget_watchdog(agent: Any, run_id: str) -> Optional[threading.Timer]:
    """Cooperatively hard-stop a public run at its wall-clock budget."""
    try:
        budget_seconds = float(getattr(agent, "run_budget_seconds", None))
    ...
    def expire() -> None:
        logger.warning(
            ...
            run_id, budget_seconds,
        )
        try:
            request_hard_interrupt(
                agent, f"API run wall-clock budget exhausted after {budget_seconds:.0f}s",
            )
        except Exception:
            logger.exception("[api_server] run %s budget interrupt failed", run_id)

    watchdog = threading.Timer(budget_seconds, expire)
...
def _reap_disconnected_agent_processes(
    ...
    process_session_key = getattr(agent, "_gateway_turn_process_session_key", "")
    process_epoch_key = getattr(
        agent, "_gateway_turn_process_epoch_key", process_session_key or process_task_id,
    )
    ...
    threading.Thread(
        ...
        kwargs={
            "source": source, "session_key": process_session_key,
...
def _publish_turn_process_ownership(agent: Any, task_id: str, *, session_key: str = "") -> None:
    ...
    agent._gateway_turn_process_baseline = process_registry.snapshot_running_ids(
        task_id, **({"session_key": session_key} if session_key else {}),
    )
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._reap_background_processes_on_run_completion = (
    ...
    def _readiness_work_counts(self) -> tuple[int, int, int]:
        """Return bounded work counts from live execution state, not the poll ledger."""
    ...
    def _resolve_reap_background_processes_on_run_completion() -> bool:
        try:
            from hermes_cli.config import cfg_get, load_config
            raw = cfg_get(
                load_config(), "gateway", "api_server",
                "reap_background_processes_on_run_completion", default=False,
            )
        except Exception:
            return False
