...
def build_voice_stream_callback(chat_id):
    """Return a sentence-buffered VoIP bridge stream callback, or ``None``."""
    match = re.match(r"^webhook:(?:call-events|incoming-call):(.+)$", chat_id or "")
    if not match:
        return None
    call_id = match.group(1)
    try:
        from hermes_cli.config import load_config_readonly
        provider = ((load_config_readonly().get("mcp_servers") or {}).get("voip-bridge") or {})
        url = (provider.get("url") or "").rstrip("/")
        if not url:
            return None
        endpoint = url.rsplit("/mcp", 1)[0] + "/stream"
        auth = (provider.get("headers") or {}).get("Authorization", "")
    except Exception:
        return None
    ...
    oob_block = re.compile(r"\[OUT-OF-BAND USER MESSAGE[^\]]*\]\s*")
    leading_bracket = re.compile(r"^\s*\[[^\]]*\]\s*")

    def flush(end=False):
        nonlocal buffer, last_flush
        text = leading_bracket.sub("", oob_block.sub("", "".join(buffer))).strip()
        buffer = []
        last_flush = time.monotonic()
        if not text:
            return
        request = urllib.request.Request(
            endpoint,
            data=json.dumps({"call_id": call_id, "delta": text, "end": end}).encode(),
    ...
    def callback(delta):
        ...
        with lock:
            ...
            text = "".join(buffer)
            now = time.monotonic()
            if len(text) >= 300 or sentence_boundary.search(text) or now - last_flush > 1.5:
                flush()
...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def _dispatch_agent_run(self, request, route_config: dict, route_name: str, profile, payload: Any, prompt: str,
        ...
        keep_alive = getattr(self, "_webhook_keep_alive", None)
        if keep_alive is None:
            keep_alive = self._webhook_keep_alive = set()
        if chat_key and request.headers.get("X-Webhook-End") != "1":
            keep_alive.add(session_chat_id)
        else:
            keep_alive.discard(session_chat_id)
    ...
    async def _end_webhook_session(self, event: "MessageEvent", session_chat_id: str) -> None:
        ...
        if session_chat_id in getattr(self, "_webhook_keep_alive", set()):
            logger.debug("[webhook] Keeping session %s alive (call in progress)", session_chat_id)
            return
