...
class TurnRunner:
    ...
    def _run_conversation_with_approval(self, agent, agent_history, observed_group_context,
        ...
        try:
            ...
            # VoIP webhook sessions stream deltas to the bridge so TTS can
            # overlap generation instead of waiting for the full reply.
            try:
                from gateway.platforms.webhook import build_voice_stream_callback
                voice_cb = build_voice_stream_callback(
                    getattr(getattr(ctx, "source", None), "chat_id", None)
                )
                if voice_cb is not None:
                    kwargs["stream_callback"] = voice_cb
            except Exception as exc:
                logger.debug("voice stream callback setup failed: %s", exc)
