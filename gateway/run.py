...
def _reap_gateway_turn_processes(
    ...
    try:
        ...
        killed = process_registry.kill_started_since(task_id, process_baseline, **options)
    except Exception:
...
class GatewayRunner(
    ...
    def _init_session_db(self) -> None:
        ...
        if self._session_db is not None:
            try:
                # A restart destroys api_server and cron request owners. Close
                # their orphaned rows before normal retention maintenance.
                api_orphans = self._session_db._db.sweep_orphaned_sessions(
                    max_idle_seconds=1.0, sources=("api_server", "cron"),
                )
                if api_orphans:
                    logger.info(
                        "Closed %d api_server/cron session row(s) orphaned by a previous gateway process",
                        len(api_orphans),
                    )
                from hermes_cli.config import load_config as _load_full_config
    ...
    _HONCHO_CACHE_BUSTING_MEMO: dict[
        tuple[str, tuple[int, int, int, int, int] | None], dict[str, Any]
    ] = {}
