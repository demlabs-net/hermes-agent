...
class TestSweepOrphanedSessions:
    ...
    def test_ephemeral_gateway_sources_can_be_swept_after_restart(self, db):
        stale = time.time() - 60
        _make_session(db, "orphaned-api-run", source="api_server", started_at=stale, message_at=stale)
        _make_session(db, "orphaned-cron-run", source="cron", started_at=stale, message_at=stale)
        _make_session(db, "persistent-telegram-session", source="telegram", started_at=stale, message_at=stale)
