...
def test_successful_mutation_resets_prior_failure_streaks_before_revalidation():
    controller = ToolCallGuardrailController(ToolCallGuardrailConfig(
        hard_stop_enabled=True, exact_failure_warn_after=2, exact_failure_block_after=3,
    ...
    for _ in range(2):
        ...
        controller.after_call("terminal", render_args, json.dumps({"exit_code": 1}), failed=True)
    assert controller.after_call(
        "patch", {"path": "page.html", "old_string": "bad", "new_string": "fixed"},
        json.dumps({"success": True}), failed=False,
    ).action == "allow"
    assert controller.before_call("terminal", render_args).action == "allow"
    decision = controller.after_call(
        "terminal", render_args, json.dumps({"exit_code": 1}), failed=True,
    )
