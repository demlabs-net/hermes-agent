...
class SessionMaintenanceMixin:
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        Records the last run timestamp and retention policy in state_meta so subsequent calls within
        ``min_interval_hours`` no-op only while that policy is unchanged. Tightening retention therefore
        takes effect at the next startup. Designed to be called once at startup from long-lived entrypoints.
        ...
        try:
            ...
            if (
                since_prune is not None
                and last_policy == policy_key
                and since_prune < min_interval_hours * 3600
            ):
                result["skipped"] = True
                return result
