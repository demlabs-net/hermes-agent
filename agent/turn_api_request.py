...
def build_api_request(
    ...
    api_start_time: Any, effective_task_id: Any, turn_id: Any, user_message: Any,
    ...
    # Once an explicitly configured completion contract has been repeatedly
    # misdirected through searches, constrain a single request to its terminal tool.
    try:
        from agent.terminal_tool_guard import (
            forced_terminal_tool_after_repeated_searches, terminal_tool_request_scope,
        )
        forced_tool = forced_terminal_tool_after_repeated_searches(
            agent=agent, user_message=user_message, messages=messages,
            current_turn_user_idx=current_turn_user_idx,
        )
        request_names = {
            str((tool.get("function") or {}).get("name") or tool.get("name") or "")
            for tool in (api_kwargs.get("tools") or []) if isinstance(tool, dict)
        }
        if forced_tool in request_names:
            if agent.api_mode == "anthropic_messages":
                api_kwargs["tool_choice"] = forced_tool
            elif agent.api_mode == "chat_completions":
                api_kwargs["tool_choice"] = {"type": "function", "function": {"name": forced_tool}}
                scope = terminal_tool_request_scope(
                    messages=api_kwargs.get("messages") or [], required_tool=forced_tool,
                )
                api_kwargs["tools"] = [
                    tool for tool in (api_kwargs.get("tools") or [])
                    if str((tool.get("function") or {}).get("name") or tool.get("name") or "") in scope
                ]
            logger.warning(
                "forcing required terminal tool after repeated misdirection: %s (request tools=%d)",
                forced_tool, len(api_kwargs.get("tools") or []),
            )
    except Exception:
