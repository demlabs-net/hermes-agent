...
# Toggle vocabulary → request vocabulary for graduated models; the binary-only
...
def resolve_lmstudio_effort(reasoning_config: Optional[dict], allowed_options: Optional[List[str]]) -> Optional[str]:
    ...
    if allowed_options:
        ...
        # The chat-completions endpoint rejects literal "on"/"off" even when
        # model metadata publishes only those options. Omit an enabled binary
        # setting so the model default applies; "none" explicitly disables it.
        if "on" in published and published.issubset({"off", "on"}):
            if effort == "none":
                return "none" if "off" in published else None
            return None
        if effort not in {_LM_EFFORT_ALIASES.get(opt, opt) for opt in published}:
            return None
    return effort
