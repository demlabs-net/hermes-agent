...
class FinalResponseVerdict:
    ...
    terminal_tool_guard_nudges: Any
...
def finish_text_response(
    ...
    def _verdict(action: str, result: Optional[Dict[str, Any]] = None) -> FinalResponseVerdict:
        return FinalResponseVerdict(
            ...
            terminal_tool_guard_nudges=terminal_tool_guard_nudges,
    ...
    try:
        from agent.terminal_tool_guard import build_required_terminal_tool_nudge
        terminal_nudge = build_required_terminal_tool_nudge(
    ...
    if terminal_nudge:
        ...
        interim_msg = agent._build_assistant_message(assistant_message, "terminal_tool_required")
        interim_msg["_terminal_tool_guard_synthetic"] = True
        append_message(messages, interim_msg)
        append_message(messages, {
            "role": "user", "content": terminal_nudge,
            "_terminal_tool_guard_synthetic": True,
        })
        agent._session_messages = messages
        agent._emit_status("⚠️ Required completion report is missing — continuing the task")
        final_response = None
        return _verdict("continue")
    ...
    _stall_continue_intent = (
        ...
        and codex_ack_continuations < max(0, getattr(agent, "_intent_ack_max_continuations", 2))
