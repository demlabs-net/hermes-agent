...
def settle_unrecovered_error(
    ...
    if retry_count >= max_retries:
        ...
        if not _retry.primary_recovery_attempted and agent._try_recover_primary_transport(
            ...
            # The rebuilt transport gets one final probe, not a second full
            # retry cycle. Resetting to zero doubled physical attempts.
            retry_count = max(0, max_retries - 1)
            # Re-open fallback state so a follow-on 429 can still activate fallback_providers.
