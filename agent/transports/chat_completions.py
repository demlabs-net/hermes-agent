...
def _promote_tool_images_to_user_messages(
    ...
    """Move images from strict tool messages into a synthetic user message."""
    ...
    def flush_pending(next_message: dict[str, Any] | None = None) -> bool:
        ...
        if not pending_parts:
            return False
        visual_parts = [
            {"type": "text", "text": "Visual payload returned by the preceding tool result(s)."},
            *pending_parts,
        ]
        pending_parts = []
        if next_message is not None and next_message.get("role") == "user":
            ...
            if isinstance(existing, list):
                ...
                next_message["content"] = [*visual_parts, {"type": "text", "text": existing}]
            else:
    ...
    for original in messages:
        ...
        if msg.get("role") == "tool" and isinstance(msg.get("content"), list):
            ...
            if image_parts:
                ...
                msg = dict(msg)
                msg["content"] = "\n\n".join(text_parts) or "[image content follows in a user message]"
                pending_parts.extend(image_parts)
        if msg.get("role") != "tool" and pending_parts:
            copied = dict(msg)
            if flush_pending(copied):
                msg = copied
        output.append(msg)
    flush_pending()
    return output
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(self, messages: list[dict[str, Any]], **kwargs) -> list[dict[str, Any]]:
        ...
        sanitized = (
            messages if all(s is None for _, s in sanitized_pairs)
            else [m if s is None else s for m, s in sanitized_pairs]
        )
        if kwargs.get("supports_vision_tool_messages") is False:
    ...
    def build_kwargs(
        ...
        sanitized = self.convert_messages(
            messages,
            model=model,
            supports_vision_tool_messages=params.get("supports_vision_tool_messages", True),
        )
