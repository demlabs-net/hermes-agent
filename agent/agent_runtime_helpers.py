...
def bridge_deferred_tool_call(agent, tool_call: Any) -> str | None:
    ...
    try:
        ...
        _ra().logger.debug("Deferred tool scope lookup failed", exc_info=True)
        return None
...
def repair_tool_call(agent, tool_name: str) -> str | None:
    ...
    for c in cands:
        if c and c in agent.valid_tool_names:
            return c
    # An exact registry hit is unavailable/deferred, not a typo. Fuzzy repair
    # could otherwise change a read into an unrelated mutating tool.
    try:
...
_ACK_ACTION_MARKERS = (
    "look into", "look at", "inspect", "continue", "scan", "check", "analyz", "review", "explore", "read", "open",
    "run", "test", "fix", "debug", "search", "find", "walkthrough", "report back", "summarize",
    "продолж", "аудит", "провер", "откро", "запущ", "исслед", "проанализ",
)
...
def looks_like_codex_intermediate_ack(
    ...
    requirement. Default codex mode preserves the historical prior-tool suppression;
    explicit all-mode opt-in also catches bounded acknowledgements after tools."""
    if require_workspace and any(isinstance(msg, dict) and msg.get("role") == "tool" for msg in messages):
        return False
    ...
    if not assistant_text or (max_chars and len(assistant_text) > max_chars):
        return False
...
__all__ = [
    ...
    "switch_model", "invoke_tool", "bridge_deferred_tool_call", "repair_tool_call", "sanitize_api_messages",
